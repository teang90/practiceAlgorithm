...
public class 가장많이받은선물 {
    ...
    public static void main(String[] args) {
        ...
        가장많이받은선물 r = new 가장많이받은선물();
        int solution = r.solution(friends, gifts);
    ...
    public int solution(String[] friends, String[] gifts) {
        ...
        for (int i = 0; i < friends.length; i++)
            nameIdxMap.put(friends[i], i);

        int[] giftIdx = new int[friends.length];
        int[][] giftRelation = new int[friends.length][friends.length];
        for (int i = 0; i < gifts.length; i++) {
            String[] split = gifts[i].split(" ");
            int senderIdx = nameIdxMap.get(split[0]);
            int receiverIdx = nameIdxMap.get(split[1]);

            giftIdx[senderIdx]++;
            giftIdx[receiverIdx]--;
            giftRelation[senderIdx][receiverIdx]++;
        }
        ...
        for (int i = 0; i < friends.length; i++) {
            ...
            for (int j = 0; j < friends.length; j++) {
                ...
                if(giftRelation[i][j] == giftRelation[j][i] && giftIdx[i] > giftIdx[j]) {
                    sumGift++;
                }
            }

            if(answer < sumGift) answer = sumGift;
        }
...
// 둘 사이의 관계에서 선물을 더 많이 보낸사람이 선물을 하나씩 받는다.
//
/**
...
예를 들어 A가 B에게 선물을 5번 줬고, B가 A에게 선물을 3번 줬다면 다음 달엔 A가 B에게 선물을 하나 받습니다. ㅡ
...
이때, 다음 달에 가장 많은 선물을 받는 친구가 받을 선물의 수를 return 하도록 solution 함수를 완성해 주세요.
